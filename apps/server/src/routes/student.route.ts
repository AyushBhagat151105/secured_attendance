...
export const studentModule = new Elysia({ prefix: "/api/student" })
  ...
  // Real-time WebSocket connection for student device & session events
  .ws("/ws", {
    async open(ws) {
      ...
      logger.info("Student connected to real-time WebSocket gateway", { userId: studentUserId });
    ...
    close() {
      logger.info("Student disconnected from real-time WebSocket gateway");
    },
  });
