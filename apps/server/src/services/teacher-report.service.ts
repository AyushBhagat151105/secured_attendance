...
export const teacherReportModule = new Elysia({ prefix: "/reports" })
  ...
  .get("/sessions", async ({ request }: any) => {
  ...
  .get("/sessions/:id/map", async ({ request, params }: any) => {
  ...
  .get("/sessions/:id", async ({ request, params }: any) => {
    const session = await auth.api.getSession({ headers: request.headers });
    if (!session) return status(401, { message: "Unauthorized" });
  ...
  .get("/subjects", async ({ request }: any) => {
    const session = await auth.api.getSession({ headers: request.headers });
    if (!session) return status(401, { message: "Unauthorized" });
  ...
  .get("/export/:subjectId", async ({ request, params, set }: any) => {
    const session = await auth.api.getSession({ headers: request.headers });
    if (!session) return status(401, { message: "Unauthorized" });
