...
- `AGENTS.md:79` specifies: _"Strict Mode: TypeScript strict mode is enabled. NEVER use `any`."_
...
`AGENTS.md:85` specifies: _"Maximum 300 lines per file. If a file grows larger, split it into smaller, cohesive modules."_
...
- `AGENTS.md:40` specifies: _"Apps must NEVER import from each other directly."_
...
1. **Offline Mode vs. Anti-Proxy Security**:
   ...
   - _Question_: Should offline scan queuing be removed, or should it require Bluetooth (BLE) attestation from the teacher device?
2. **Pluggable Security Module Rollout**:
   ...
   - _Question_: Should the abstract plugin interface in `apps/server/src/plugins/` be stubbed out now, or deferred until after the core attendance loop is stabilized?
